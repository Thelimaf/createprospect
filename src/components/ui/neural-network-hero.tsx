...
declare module '@react-three/fiber' {
  interface ThreeElements {
    cPPNShaderMaterial: {
      ref?: React.Ref<THREE.ShaderMaterial & { iTime: number; iResolution: THREE.Vector2 }>;
      iTime?: number;
...
function ShaderBackground() {
  ...
  if (!isVisible) {
    return (
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-primary/20 via-background to-accent/10" />
    );
  }
  ...
  return (
    ...
    >
      <Suspense fallback={<div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-background to-accent/10" />}>
        <Canvas camera={camera} dpr={[1, 1.5]} gl={{ antialias: false, alpha: true }}>
          <ShaderPlane />
        </Canvas>
      </Suspense>
...
function useSplitTextAnimation(
  ...
  useGSAP(
    () => {
      ...
      document.fonts.ready.then(() => {
        ...
        // Get all child elements or create spans for text nodes
        const children = Array.from(element.children);
        
        if (children.length === 0) {
          // If no children, wrap text content in spans by words
        ...
        const animatableElements = Array.from(element.querySelectorAll('span, .animate-line'));
...
export default function NeuralNetworkHero({
  ...
  useSplitTextAnimation(sectionRef, headerRef);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center text-center">
        ...
        >
          <span className="inline-block animate-line">{title}</span>
          {titleHighlight && (
            <span className="block gradient-text animate-line">{titleHighlight}</span>
          )}
        </h1>
